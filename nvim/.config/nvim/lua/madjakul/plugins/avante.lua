...
return {
    ...
    -- Build from source using Cargo. The default `make` tries to download
    -- prebuilt binaries from GitHub which can fail (rate limits, missing arch).
    -- Building from source requires Rust/Cargo (installed by the install script).
